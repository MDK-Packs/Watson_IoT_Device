...
# Upstream repository
UPSTREAM_URL=https://api.github.com/repos/ibm-watson-iot/iot-embeddedc
UPSTREAM_TAG=30f4ffb
...
"${PACKCHK}" "${PACK_BUILD}/${PACK_VENDOR}.${PACK_NAME}.pdsc" \
  ...
  -i "${CMSIS_PACK_ROOT}/.Web/MDK-Packs.Paho_MQTT.pdsc" \
  -i "${CMSIS_PACK_ROOT}/.Web/MDK-Packs.cJSON.pdsc" \
  -x M396 -x M382 \
  -n PackName.txt
errorlevel=$?
if [ $errorlevel -ne 0 ]; then
